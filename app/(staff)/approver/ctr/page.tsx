...
export default async function ApproverCtrQueuePage() {
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-xl font-bold text-zinc-900">CTR Approval Queue</h1>
            {ctrList.length > 0 && (
              <span className="inline-flex items-center gap-1 rounded-full border border-sky-200 bg-sky-50 px-2.5 py-0.5 text-xs font-semibold text-sky-800">
          ...
          <p className="mt-0.5 text-sm text-zinc-500">
            Customer type reclassifications awaiting your final decision.
          </p>
        </div>
      </div>
      ...
      {ctrList.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-xl border border-dashed bg-white py-20 text-center">
          <EmptyStateLogo />
          <h2 className="mt-4 text-base font-semibold text-zinc-900">No pending CTRs</h2>
          <p className="mt-1 text-sm text-zinc-500">
            No type reclassification requests awaiting your approval.
          </p>
        </div>
      ) : (
              ...
              <li key={ctr.id}>
                ...
                >
                  <div className="min-w-0 space-y-1">
                    ...
                    {ctr.reason && (
                      <p className="text-xs text-zinc-400 truncate max-w-sm">{ctr.reason}</p>
                    )}
                  </div>
