...
const agentSubmitSchema = z.object({
  agentAccountSpecialist: z.string().min(1, "Account Specialist is required").max(255),
...
export async function PATCH(
  ...
  const {
    ...
    agentSalesSpecialist, salesSupportAccountType, agentTpcFirst, agentTpcLast,
  ...
  if (existingColumns.has("agent_account_specialist")) {

    updatePayload.agentAccountSpecialist = agentAccountSpecialist || null;

  } else {
    // Backward compatibility: split name into first/last if old columns exist
    if (existingColumns.has("agent_account_specialist_first")) {
      const parts = agentAccountSpecialist.trim().split(/\s+/);
      updatePayload.agentAccountSpecialistFirst = parts[0] || null;
      updatePayload.agentAccountSpecialistLast = parts.slice(1).join(" ") || null;
    }
  }
  ...
  if (existingColumns.has("sales_support_account_type")) {

    updatePayload.salesSupportAccountType = salesSupportAccountType;
