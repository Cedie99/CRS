...
export function AgentFillOutForm({ cisId, initialCustomerType = "", initialAccountType = null, initialOtherRequirements = [], tradeName, managerName, agentType = "sales_agent" }: AgentFillOutFormProps) {
  ...
  const STORAGE_KEY = `agent_fill_draft_${cisId}`;

  const [fields, setFields] = useState<FormFields>({
    agentAccountSpecialist: "",
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (saved) {
        ...
        if (data.otherRequirements && Array.isArray(data.otherRequirements)) setOtherRequirements(data.otherRequirements);
      }
    } catch (e) {
  ...
  }, [STORAGE_KEY, fields, otherRequirements]);



  const requiredFields = [
    fields.agentAccountSpecialist.trim(),
  ...
  const missingRequiredCount = totalRequiredFields - requiredFilledCount;
  ...
  function validate(): boolean {
    ...
    if (!fields.agentAccountSpecialist.trim()) errs.agentAccountSpecialist = "Account Specialist is required";

    if (!fields.agentSalesSpecialist.trim()) errs.agentSalesSpecialist = "Sales Specialist is required";
    if (!fields.accountType) errs.accountType = "Account type is required";
    ...
    try {
      ...
      const res = await fetch(`/api/cis/${cisId}/agent-submit`, {
        ...
        body: JSON.stringify({
          ...
          agentAccountSpecialist: fields.agentAccountSpecialist,
          ...
          salesSupportAccountType: fields.accountType,
      ...
      <CardHeader className="overflow-hidden rounded-t-xl border-b border-sky-100/80 pb-4">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          ...
          <div className="inline-flex items-center gap-2 self-start rounded-full border border-sky-200 bg-white px-3 py-1.5 text-xs font-medium text-sky-800">
            ...
            {requiredFilledCount === totalRequiredFields ? (
        ...
        <section className={`relative rounded-xl border border-zinc-200 bg-white/80 p-4 sm:p-5${!isReadyToSubmit ? " agent-step-shine" : ""}`}>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="space-y-1.5 sm:col-span-2">
              ...
              <Label htmlFor="acc-spec">Account Specialist *</Label>
            ...
            <Input
              ...
              value={fields.agentAccountSpecialist}

              onChange={(e) => setField("agentAccountSpecialist", e.target.value)}

              placeholder="Enter 'In-House' or full name"
            ...
            {errors.agentAccountSpecialist && (

              <p className="text-xs text-red-600">{errors.agentAccountSpecialist}</p>
                ...
                {ACCOUNT_TYPE_OPTIONS.map((opt) => (
                  ...
                  >
                    <input
                      ...
                      name="accountType"
                      value={opt.value}
                      checked={fields.accountType === opt.value}
                      onChange={() => setField("accountType", opt.value)}
                      disabled={isLoading}
